...
| M9c — RT-safety, performance, release | ✅ done (hands-on checks pending) | Landed 2026-07-08, scoped-then-built (`c9640a3` P0 docs). **P1** `8e2351d`: check.sh gate → pluginval strictness 10 + auval (both green). **P2** `987ea35`: audio path made alloc-free (param table, stack-buffer ids, raw-byte MIDI) + malloc-guard test (0 allocations over 200 busy blocks); headphones out resolved = silkscreen in all builds. **P3** `70ebf97`: `--bench` CPU gate in check.sh, fails > 10 % @48k/128 — measured **4.0 %**. Swap hardening `c47d191`: statecheck quiet-window race fixed (deterministic 6/6). **P4** `f8ea8d0`: digital-only **MIDI CLK** jack (append `midi.clock.out`) — 24 ppqn → divided 10 V pulses, Settings→MIDI rate, START/STOP semantics, JUCE-free + 4 tests. **P5** `c6b03d5`: right-click CC learn (MidiCcMap ring + 30 Hz drain, MIDIMAP state child preserves the desk map across preset loads) + 5 tests. **P6**: `10-release-signing.md` (codesign/notarize recipe; branding blocker restated). check.sh ALL GREEN — 105/105, pluginval 10 SUCCESS, auval PASS, CPU 4 %. **No sonic change** (render hash bit-identical). `renders/m9c-panel-screenshot.png` — **user eye check + KeyLab 37 hands-on pending** |
| M9d — DAW integration: Ableton Live 11 smoke | 🔶 in progress | Scoped 2026-07-11. VST3 status reviewed: built + pluginval 10 green since M0, arm64, ad-hoc signed (fine locally), ~200 params, any-block-size safe (64-sample internal chunking) — but never installed nor loaded in a real host. Scope: check.sh installs the VST3 to `~/Library/Audio/Plug-Ins/VST3/` (mirrors the AU rsync); then a hands-on Live 11 session — instantiate/GUI resize, KeyLab 37 play (doubles as the pending M9a hands-on), sound parity vs standalone, Live-set save→quit→reopen state, automation lanes, CC-learn, MIDI clock via transport, buffer sizes 64–1024, multi-instance CPU, track Freeze/Flatten. Verdict → `00-LOG.md` |
...
STATUS: all phases done 2026-07-08 — see the milestone table for commits
and evidence; user hands-on checks pending)** —
